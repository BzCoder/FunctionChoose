 ...
 * @date : 2019/1/7 11:22
 */
public class HuZhouCityServiceData implements CityData {
    private List<FunctionItem> data;

    public HuZhouCityServiceData() {
        data = new ArrayList<>();
        data.add(new FunctionItem("自然气象", "", 0, ServiceEnum.TITLE));
        data.add(new FunctionItem("天气预报", "https://app.zjzwfw.gov.cn/zjjssdk/tqyb/weather.jsp?citycode=101210201&123", R.mipmap.city_service_weather_forecast, ServiceEnum.FOUR_ONE_LINE));
		...
		data.add(new FunctionItem("客运时刻", "http://s.huzfb.com/bus/", R.mipmap.city_service_departure_schedule, ServiceEnum.FOUR_ONE_LINE));
